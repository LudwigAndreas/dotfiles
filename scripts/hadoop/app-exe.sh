...
upload_to_hdfs() {
    ...
    # Upload JAR files
    hdfs dfs -put "$LOCAL_PROJECT_DIR"/*.jar "$HDFS_BASE_PATH/lib/"
    
    # Upload workflow.xml
    hdfs dfs -put "$LOCAL_PROJECT_DIR/$WORKFLOW_XML_NAME" "$HDFS_BASE_PATH/"
    
    # Upload shell scripts
    hdfs dfs -put "$LOCAL_PROJECT_DIR"/*.sh "$HDFS_BASE_PATH/scripts/"
}
